...
class User{
	...
	public function new_user($email,$password,$lastname,$firstname,$access,$contactno){
		...
		$data = [
			[$lastname,$firstname,$email,$password,$NOW,$NOW,'1',$access,$contactno],
		];
		$stmt = $this->conn->prepare("INSERT INTO tbl_users (user_lastname, user_firstname, user_email, user_password, user_date_added, user_time_added, user_status, user_access, contactno) VALUES (?,?,?,?,?,?,?,?,?)");
		try {
	...
	public function update_user($lastname,$firstname, $access, $id, $contactno){
		
		/* Setting Timezone for DB */
		$NOW = new DateTime('now', new DateTimeZone('Asia/Manila'));
		$NOW = $NOW->format('Y-m-d H:i:s');

		$sql = "UPDATE tbl_users SET user_firstname=:user_firstname,user_lastname=:user_lastname,user_date_updated=:user_date_updated,user_time_updated=:user_time_updated,user_access=:user_access,contactno=:contactno WHERE user_id=:user_id";

		$q = $this->conn->prepare($sql);
		$q->execute(array(':user_firstname'=>$firstname, ':user_lastname'=>$lastname,':user_date_updated'=>$NOW,':user_time_updated'=>$NOW,':user_access'=>$access,':user_id'=>$id,':contactno'=>$contactno));
		return true;
	}
	...
	public function change_contactno($id,$contactno){
		
		/* Setting Timezone for DB */
		$NOW = new DateTime('now', new DateTimeZone('Asia/Manila'));
		$NOW = $NOW->format('Y-m-d H:i:s');

		$sql = "UPDATE tbl_users SET contactno=:contactno,user_date_updated=:user_date_updated,user_time_updated=:user_time_updated WHERE user_id=:user_id";

		$q = $this->conn->prepare($sql);
		$q->execute(array(':contactno'=>$contactno,':user_date_updated'=>$NOW,':user_time_updated'=>$NOW,':user_id'=>$id));
		return true;
	}
	...
	function get_user_contactno($id){
		$sql="SELECT contactno FROM tbl_users WHERE user_id = :id";	
		$q = $this->conn->prepare($sql);
		$q->execute(['id' => $id]);
		$contactno = $q->fetchColumn();
		return $contactno;
	}
